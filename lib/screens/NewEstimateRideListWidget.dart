...
class NewEstimateRideListWidgetState extends State<NewEstimateRideListWidget> {
  ...
  void init() async {
    sourceIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(devicePixelRatio: 1.5), SourceIcon);
    destinationIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(devicePixelRatio: 1.5), DestinationIcon);
    driverIcon = await BitmapDescriptor.fromAssetImage(ImageConfiguration(devicePixelRatio: 1.5), DriverIcon);
    getServiceList();
    getCurrentRequest();
    mqttForUser();
  ...
  Future<void> setPolyLines({required LatLng sourceLocation, required LatLng destinationLocation, LatLng? driverLocation}) async {
    ...
    var request = PolylineRequest(
      ...
      destination: rideRequest != null && (rideRequest!.status == ACCEPTED || rideRequest!.status == ARRIVING || rideRequest!.status == ARRIVED)
    ...
    var result = await polylinePoints.getRouteBetweenCoordinates(
      request: request,
      googleApiKey: GOOGLE_MAP_API_KEY, // Asegúrate de que GOOGLE_MAP_API_KEY es una constante válida con tu API key
    );
